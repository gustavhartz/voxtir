...
export function addFiles(
  ...
  createFolder(zip, '_rels').file('.rels', getBinaryData(relsXml), {
    createFolders: false,
  });
  let folder = createFolder(zip, 'word')
  ...
  folder = createFolder(folder, '_rels').file(
    'document.xml.rels',
    getBinaryData(documentXmlRels),
    {
      createFolders: false,
    }
  );
