...
const routes: Route[] = [
  { name: 'Projects', path: '/', icon: AiOutlineFolder, activeIcon: AiFillFolderOpen },
  { name: 'Profile', path: '/me', icon: FaRegUserCircle, activeIcon: FaUserCircle },
];
  ...
  return (
    <div className="px-3">
      {routes.map((route, index) => (
        ...
        >
          {currentRoute === route.path ? <route.activeIcon size={30} className="mr-3" /> : <route.icon size={30} className="mr-3" />}
            ...
            <div className="w-96 flex flex-col min-h-full h-full bg-white border-r-2 border-gray-100">
                ...
                <div className="flex flex-col items-center py-4 px-4">
                    <div className="bg-gray-800 w-full h-14 flex justify-between px-4 items-center mb-4 rounded-lg drop-shadow-sm">
                      <p className="text-lg text-white font-medium">Upgrade to <span className="bg-black text-white px-2 py-1 rounded-lg ml-1">PRO</span></p>
                      <div className="bg-white p-1 rounded-full shadow-sm">
                        <FiArrowUpRight size={28} />
                      </div>
                    </div>
                    <button onClick={() => logout()} className="text-black font-medium flex items-center text-lg"><IoMdLogOut size={26} className="mr-2" /> Sign Out</button>
                </div>
            </div>
        );
    }
