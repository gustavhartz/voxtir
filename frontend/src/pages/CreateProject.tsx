...
const CreateProject = ({ token }: { token: string }) => {
  ...
  return (
    <div className="p-6 w-full  -ml-1 transition-all lg:flex lg:items-center lg:justify-center">
        <form onSubmit={formik.handleSubmit} className="drop-shadow-sm bg-white border p-6 w-full rounded-md text-black lg:w-1/2">
        <h2 className="text-3xl text-gray-800 font-bold mb-8">Create new project</h2>
        ...
        <button disabled={!formik.isValid || !formik.dirty} type="submit" className="disabled:cursor-not-allowed bg-gray-900 opacity-100 hover:black disabled:opacity-20 duration-500 transition-opacity w-full py-2 mt-4 rounded-lg text-white disabled:text-gray-300 font-medium">Create</button>
      </form>
    </div>
  );
};
